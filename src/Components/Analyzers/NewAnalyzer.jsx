...
export const NewAnalyzer = ({ sendPosition, ...props }) => {
  ...
  const JD = 367 * year - Math.trunc((7 * (year + Math.trunc((month + 9) / 12.0))) / 4.0) + 
            Math.trunc((275 * month) / 9.0) + day + 1721013.5

  // Astronomical calculations
  const T = (JD - 2451545.0) / 36525
  const LL = (280.46646 + 36000.76983 * T + 0.0003032 * T ** 2) % 360
  const M = (357.52911 + 35999.05029 * T - 0.000153 * T ** 2) % 360
  const C = (1.914602 - 0.004817 * T - 0.000014 * T ** 2) * Math.sin(Math.radians(M)) +
  ...
  const epxlong0 = 23 + 26/60 + 21.448/3600 - (46.815/3600) * T - 
  ...
  const depxlong = (9.2/3600) * Math.cos((Q * Math.PI)/180) +
  ...
  return (
    <div id="analyzerPrincipalContainer">
      ...
      <div
        ...
        ref={ref}
        style={{
          backgroundImage: newDate > refDate
            ? `url(${errorBackground})`
